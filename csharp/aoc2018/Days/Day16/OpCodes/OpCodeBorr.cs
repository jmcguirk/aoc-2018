...
{
    public class OpCodeBorr : IOpCode
    {
        public bool Execute(int[] arguments, int[] registerState)
        {
            ...
            //Log.WriteLine("Borr " + registerA + " " + registerB + " " + targetRegister);
            ...
            if (registerA < registerState.Length && registerA >= 0 && registerB < registerState.Length && registerB >= 0)
            {
                ...
                //Log.WriteLine("Storing " + (valA | valB) + "(which is the bitwise OR of " + valA + "," + valB + ") into register " + targetRegister);
